...
static pthread_mutex_t OPCUA_client_wakeup_mutex = PTHREAD_MUTEX_INITIALIZER;

/* wakeup connection thread */
static pthread_cond_t OPCUA_thread_wakeup_cond = PTHREAD_COND_INITIALIZER;
...
{{
    ...
    OPCUA_stop_thread = 1;
    lock_result = pthread_mutex_lock(&OPCUA_client_wakeup_mutex);
    if (lock_result == 0){{
        /* unblock connection thread so that it can stop normally */
        pthread_cond_signal(&OPCUA_thread_wakeup_cond);
        pthread_mutex_unlock(&OPCUA_client_wakeup_mutex);
    }} else {{
        LogError("OPC-UA cleanup: FAILED to lock mutex, error=%d\n", lock_result);
    }}
...
static void* __OPCUA_thread_proc(void *arg) {{
    ...
    while((rc = pthread_mutex_lock(&OPCUA_client_wakeup_mutex)) == 0 && !OPCUA_stop_thread){{
        pthread_cond_wait(&OPCUA_thread_wakeup_cond, &OPCUA_client_wakeup_mutex);
        ...
        {{
            ...
            if (retval == UA_STATUSCODE_GOOD) {{
                ...
                LogError("OPC-UA Connection failed with status code %s\n", UA_StatusCode_name(retval));
            }}
        }}

        pthread_mutex_unlock(&OPCUA_client_wakeup_mutex);

        if(OPCUA_stop_thread) break;
    }}

    LogInfo("OPC-UA thread: exited while loop\n");

    if(!OPCUA_stop_thread){{
        /* if thread exits outside of normal shutdown, report error*/
        LogError("OPC-UA client thread exited unexpectedly, return code %d\n", rc);
    }}
    return NULL;
}}
...
#define READ_VALUE(ua_type, ua_type_enum, c_loc_name, ua_nodeid_type, ua_nsidx, ua_node_id)        \
    {{                                                                                            \
        if(!OPCUA_is_disconnected && pthread_mutex_trylock(&OPCUA_client_wakeup_mutex) == 0) {{          \
...
{{
    if (pthread_mutex_trylock(&OPCUA_client_wakeup_mutex) == 0) {{
        {publish}

...
void PLC_SetTimer(unsigned long long next, unsigned long long period)
{
	...
	/* Apply change immediately by interrupting the current cycle */
	// pthread_kill(PLC_thread, SIGUSR1);
    //
	// Impossible to interrupt clock_nanpsleep with a signal
	//
	// - pthread_kill is not implemented in Zephyr
	// - k_wakeup wouldn't work either since
	//   nanosleep is not interuptile in Zephyr
}

...
CanFestivalPath = paths.ThirdPartyPath("canfestival")  # noqa
...
# to be generated by the (now removed) configure script.

CanFestivalBuildPath = os.path.join(CanFestivalPath, "build")


def _ReadCMakeCache():
    """Parse CanFestival's CMakeCache.txt as {key: value}.

    Returns {} if the cache is missing (CanFestival not built yet).
    """
    cache = {}
    cache_path = os.path.join(CanFestivalBuildPath, "CMakeCache.txt")
...
_CMakeCache = _ReadCMakeCache()

# Pick up the target/timer/CAN driver that CanFestival was actually built with.
...
def getCFLAGS(Cpth):
    ...
    include = os.path.join(Cpth, "include")
    return ' -I"%s" -I"%s" -I"%s" -I"%s"' % (
        include,
        os.path.join(include, CanFestivalTarget),
        os.path.join(include, "timers_%s" % CanFestivalTimers),
        # CMake writes the generated config.h into the build tree.
        os.path.join(CanFestivalBuildPath, "include"))


def getLDFLAGS(Cpth):
    """Static libraries (CMake build outputs) to link into the PLC."""
    return ' "%s" "%s"' % (
        os.path.join(CanFestivalBuildPath, "src", "libcanfestival.a"),
        os.path.join(CanFestivalBuildPath, "drivers",
                     "libcanfestival_%s.a" % CanFestivalTarget))
...
class _SlaveCTN(NodeManager):
    ...
    def CTNGenerate_C(self, buildpath, locations):
        ...
        if res:
            raise Exception(res)
        return [(Gen_OD_path, getCFLAGS(CanFestivalPath))], "", False
...
class RootClass(object):
            ...
            if element["name"] == "CanFestivalInstance":
                for child in element["children"]:
                    if child["name"] == "CAN_Driver":
                        child["type"] = DLL_LIST
                        # can_drivers = DLL_LIST
                        # can_driver = child["value"]
                        # if can_drivers and can_driver not in can_drivers:
                        #     child["value"] = can_drivers[0]
        return infos
    ...
    def CTNGenerate_C(self, buildpath, locations):
            ...
            if can_driver not in can_drivers:
                if can_driver:
                    ...
                    _("Warning: %s CAN driver found in %s; rebuild CanFestival "
                      "with -DCF_CAN_DRIVER=<name>\n") % (can_driver,CanFestivalBuildPath))
        ...
        res = [(cf_main_path, getCFLAGS(CanFestivalPath))], getLDFLAGS(CanFestivalPath), True

        if can_driver is not None:
            # The CAN driver shared library is a CMake build output.
            can_driver_path = os.path.join(CanFestivalBuildPath, "drivers", can_driver_filename)

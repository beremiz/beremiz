...
class SDOEntry(object):
    def __init__(self, idx=None, subIdx=None, datatype=None, size=None, value=None):
        self.idx = idx # string
        self.subIdx = subIdx # string
        self.datatype = datatype # string
        self.size = size # string
        self.value = value # string

    def _read(self, codec):
        self.idx = codec.read_string()
        self.subIdx = codec.read_string()
        self.datatype = codec.read_string()
        self.size = codec.read_string()
        self.value = codec.read_string()
        return self

    def _write(self, codec):
        if self.idx is None:
            raise ValueError("idx is None")
        codec.write_string(self.idx)
        if self.subIdx is None:
            raise ValueError("subIdx is None")
        codec.write_string(self.subIdx)
        if self.datatype is None:
            raise ValueError("datatype is None")
        codec.write_string(self.datatype)
        if self.size is None:
            raise ValueError("size is None")
        codec.write_string(self.size)
        if self.value is None:
            raise ValueError("value is None")
        codec.write_string(self.value)

    def __str__(self):
        return "<%s@%x idx=%s subIdx=%s datatype=%s size=%s value=%s>" % (self.__class__.__name__, id(self), self.idx, self.subIdx, self.datatype, self.size, self.value)

    def __repr__(self):
        return self.__str__()

class SDODataPack(object):
    def __init__(self, entries=None, slavePos=None):
        self.entries = entries # list<SDOEntry>
        self.slavePos = slavePos # uint16

    def _read(self, codec):
        _n0 = codec.start_read_list()
        self.entries = []
        for _i0 in range(_n0):
            _v0 = SDOEntry()._read(codec)
            self.entries.append(_v0)

        self.slavePos = codec.read_uint16()
        return self

    def _write(self, codec):
        if self.entries is None:
            raise ValueError("entries is None")
        codec.start_write_list(len(self.entries))
        for _i0 in self.entries:
            _i0._write(codec)

        if self.slavePos is None:
            raise ValueError("slavePos is None")
        codec.write_uint16(self.slavePos)

    def __str__(self):
        return "<%s@%x entries=%s slavePos=%s>" % (self.__class__.__name__, id(self), self.entries, self.slavePos)

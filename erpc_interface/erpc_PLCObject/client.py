...
class BeremizPLCObjectServiceClient(interface.IBeremizPLCObjectService):
    ...
    def GetSDOEntriesData(self, entries, slavePos, result):
        ...
        codec.start_write_message(erpc.codec.MessageInfo(
                type=erpc.codec.MessageType.kInvocationMessage,
                service=self.SERVICE_ID,
                request=self.GETSDOENTRIESDATA_ID,
                sequence=request.sequence))
        if entries is None:
            raise ValueError("entries is None")
        codec.start_write_list(len(entries))
        for _i0 in entries:
            _i0._write(codec)

        if slavePos is None:
            raise ValueError("slavePos is None")
        codec.write_uint16(slavePos)
        ...
        for _i0 in range(_n0):
            ...
            result.value.append(_v0)

        _result = codec.read_uint32()
        return _result

    def SetSDOTraceValues(self, entries, slavePos, result):
        assert type(result) is erpc.Reference, "out parameter must be a Reference object"
        ...
        codec.start_write_message(erpc.codec.MessageInfo(
                type=erpc.codec.MessageType.kInvocationMessage,
                service=self.SERVICE_ID,
                request=self.SETSDOTRACEVALUES_ID,
                sequence=request.sequence))
        if entries is None:
            raise ValueError("entries is None")
        codec.start_write_list(len(entries))
    ...
    def GetSDOData(self, result):
        ...
        codec.start_write_message(erpc.codec.MessageInfo(
                type=erpc.codec.MessageType.kInvocationMessage,
                service=self.SERVICE_ID,
                request=self.GETSDODATA_ID,
                sequence=request.sequence))

        # Send request and process reply.
        self._clientManager.perform_request(request)
        result.value = common.SDODataPack()._read(codec)
        _result = codec.read_uint32()
        return _result

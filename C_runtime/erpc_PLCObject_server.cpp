...
erpc_status_t BeremizPLCObjectService_service::handleInvocation(uint32_t methodId, uint32_t sequence, Codec * codec, MessageBufferFactory *messageFactory, Transport * transport)
{
    ...
    switch (methodId)
    {
        ...
        case BeremizPLCObjectService_interface::m_DebugControlId:
        {
            erpcStatus = DebugControl_shim(codec, messageFactory, transport, sequence);
            break;
        }
...
// Server shim for DebugControl of BeremizPLCObjectService interface.
erpc_status_t BeremizPLCObjectService_service::DebugControl_shim(Codec * codec, MessageBufferFactory *messageFactory, Transport * transport, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    if (err == kErpcStatus_Success)
    {
        ...
        nestingDetection = true;
#endif
        result = m_handler->DebugControl(key);
#if ERPC_NESTED_CALLS_DETECTION
        nestingDetection = false;
#endif
    ...
    if (err == kErpcStatus_Success)
    {
        ...
        // Build response message.
        codec->startWriteMessage(message_type_t::kReplyMessage, BeremizPLCObjectService_interface::m_serviceId, BeremizPLCObjectService_interface::m_DebugControlId, sequence);
